...
namespace ConcurrentStackParallel
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Task.WhenAll(Enumerable.Range(0, numParallelTasks).Select(i => Task.Factory.StartNew((state) =>
            {
                // state = i * numItems
                int index = (int)state;
            ...
            }, i * numItems, CancellationToken.None, TaskCreationOptions.DenyChildAttach, TaskScheduler.Default)).ToArray()).GetAwaiter().GetResult();


            int numTotalElements = 4 * numItems;
            int[] resultBuffer = new int[numTotalElements];
            Task.WhenAll(Enumerable.Range(0, numParallelTasks).Select(i => Task.Factory.StartNew(obj =>
            {
                int index = (int)obj;
            ...
            }, i * numItems, CancellationToken.None, TaskCreationOptions.LongRunning, TaskScheduler.Default)).ToArray()).GetAwaiter().GetResult();
